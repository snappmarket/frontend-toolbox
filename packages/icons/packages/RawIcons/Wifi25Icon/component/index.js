/**
 * THIS IS AN AUTO GENERATED FILE, CHANGES WILL NOT APPLY
 */
import * as React from 'react';
import PropTypes from 'prop-types';

const Wifi25Icon = ({ className, size }) => (
  <svg
    data-testid="Wifi25Icon"
    viewBox="0 0 24 24"
    className={className}
    style={{
      width: size * 10,
      height: size * 10,
    }}
    focusable="false"
    fill="currentColor"
  >
    <path d="M12 15c-1.653 0-3 1.347-3 3s1.347 3 3 3 3-1.347 3-3-1.347-3-3-3zM12 19c-0.55 0-1-0.45-1-1s0.45-1 1-1c0.55 0 1 0.45 1 1s-0.45 1-1 1z"></path>
    <path d="M15.666 13.247c-2.156-1.669-5.169-1.669-7.328 0l-0.397 0.306 0.612 0.791 0.397-0.306c1.797-1.391 4.306-1.391 6.103 0l0.397 0.306 0.612-0.791-0.397-0.306z"></path>
  </svg>
);

Wifi25Icon.propTypes = {
  className: PropTypes.string,
  size: PropTypes.number,
};

Wifi25Icon.defaultProps = {
  size: 1.5,
};

export default Wifi25Icon;
