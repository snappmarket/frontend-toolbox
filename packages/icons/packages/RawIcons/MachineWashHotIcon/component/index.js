/**
 * THIS IS AN AUTO GENERATED FILE, CHANGES WILL NOT APPLY
 */
import * as React from 'react';
import PropTypes from 'prop-types';

const MachineWashHotIcon = ({ className, size }) => (
  <svg
    data-testid="MachineWashHotIcon"
    viewBox="0 0 24 24"
    className={className}
    style={{
      width: size * 10,
      height: size * 10,
    }}
    focusable="false"
    fill="currentColor"
  >
    <path d="M21.281 3.497l-0.725 2.775c-2.972-0.753-5.784 0.206-8.516 1.184-1.778-0.95-3.516-1.416-5.306-1.422-0.012 0-0.025 0-0.038 0-1.178 0-2.231 0.197-3.2 0.459l-0.781-2.991-1.934 0.5 4.25 16.25c0.116 0.441 0.513 0.747 0.969 0.747h12c0.456 0 0.853-0.306 0.969-0.747l4.25-16.25-1.938-0.506zM10.725 7.922c-2.225 0.772-4.378 1.369-6.606 0.934l-0.366-1.4c1.938-0.522 4.194-0.769 6.972 0.466zM17.228 19h-10.456l-2.375-9.084c0.422 0.059 0.841 0.084 1.256 0.084 2.244 0 4.406-0.775 6.516-1.528 2.766-0.991 5.391-1.928 8.134-1.234l-3.075 11.763z"></path>
    <path d="M12 13c-0.55 0-1 0.45-1 1s0.45 1 1 1c0.55 0 1-0.45 1-1s-0.45-1-1-1zM12 14c0 0 0 0 0 0v0z"></path>
    <path d="M15 13c-0.55 0-1 0.45-1 1s0.45 1 1 1 1-0.45 1-1-0.45-1-1-1zM15 14c0 0 0 0 0 0v0z"></path>
    <path d="M9 13c-0.55 0-1 0.45-1 1s0.45 1 1 1 1-0.45 1-1-0.45-1-1-1zM9 14c0 0 0 0 0 0v0z"></path>
  </svg>
);

MachineWashHotIcon.propTypes = {
  className: PropTypes.string,
  size: PropTypes.number,
};

MachineWashHotIcon.defaultProps = {
  size: 1.5,
};

export default MachineWashHotIcon;
