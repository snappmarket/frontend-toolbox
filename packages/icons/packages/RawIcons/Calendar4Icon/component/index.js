/**
 * THIS IS AN AUTO GENERATED FILE, CHANGES WILL NOT APPLY
 */
import * as React from 'react';
import PropTypes from 'prop-types';

const Calendar4Icon = ({ className, size }) => (
  <svg
    data-testid="Calendar4Icon"
    viewBox="0 0 24 24"
    className={className}
    style={{
      width: size * 10,
      height: size * 10,
    }}
    focusable="false"
    fill="currentColor"
  >
    <path d="M0 1h5v2h-5v-2z"></path>
    <path d="M9 1h6v2h-6v-2z"></path>
    <path d="M19 1h5v2h-5v-2z"></path>
    <path d="M6 0h2v7h-2v-7z"></path>
    <path d="M16 0h2v7h-2v-7z"></path>
    <path d="M9 4h6v2h-6v-2z"></path>
    <path d="M22 4h-3v2h3v16h-15v-4.5c0-0.275-0.225-0.5-0.5-0.5h-4.5v-11h3v-2h-3c-1.103 0-2 0.897-2 2v11c0 3.859 3.141 7 7 7h15c1.103 0 2-0.897 2-2v-16c0-1.103-0.897-2-2-2zM2.1 18h3.9v3.9c-1.956-0.4-3.5-1.944-3.9-3.9z"></path>
    <path d="M11 9h2v1h-2v-1z"></path>
    <path d="M14 9h2v1h-2v-1z"></path>
    <path d="M17 9h2v1h-2v-1z"></path>
    <path d="M8 11h2v1h-2v-1z"></path>
    <path d="M11 11h2v1h-2v-1z"></path>
    <path d="M14 11h2v1h-2v-1z"></path>
    <path d="M17 11h2v1h-2v-1z"></path>
    <path d="M5 11h2v1h-2v-1z"></path>
    <path d="M8 13h2v1h-2v-1z"></path>
    <path d="M11 13h2v1h-2v-1z"></path>
    <path d="M14 13h2v1h-2v-1z"></path>
    <path d="M17 13h2v1h-2v-1z"></path>
    <path d="M5 13h2v1h-2v-1z"></path>
    <path d="M8 15h2v1h-2v-1z"></path>
    <path d="M11 15h2v1h-2v-1z"></path>
    <path d="M14 15h2v1h-2v-1z"></path>
    <path d="M17 15h2v1h-2v-1z"></path>
    <path d="M5 15h2v1h-2v-1z"></path>
    <path d="M8 17h2v1h-2v-1z"></path>
    <path d="M11 17h2v1h-2v-1z"></path>
    <path d="M14 17h2v1h-2v-1z"></path>
    <path d="M17 17h2v1h-2v-1z"></path>
    <path d="M8 19h2v1h-2v-1z"></path>
    <path d="M11 19h2v1h-2v-1z"></path>
    <path d="M14 19h2v1h-2v-1z"></path>
    <path d="M17 19h2v1h-2v-1z"></path>
  </svg>
);

Calendar4Icon.propTypes = {
  className: PropTypes.string,
  size: PropTypes.number,
};

Calendar4Icon.defaultProps = {
  size: 1.5,
};

export default Calendar4Icon;
