/**
 * THIS IS AN AUTO GENERATED FILE, CHANGES WILL NOT APPLY
 */
import * as React from 'react';
import PropTypes from 'prop-types';

const SyncWarningIcon = ({ className, size }) => (
  <svg
    data-testid="SyncWarningIcon"
    viewBox="0 0 24 24"
    className={className}
    style={{
      width: size * 10,
      height: size * 10,
    }}
    focusable="false"
    fill="currentColor"
  >
    <path d="M19.141 5h-4.141v2h6c0.553 0 1-0.447 1-1v-6h-2v3.059c-0.725-0.647-1.525-1.209-2.384-1.666-1.716-0.912-3.659-1.394-5.616-1.394-1.619 0-3.191 0.319-4.672 0.944-1.428 0.603-2.712 1.469-3.813 2.572s-1.966 2.384-2.572 3.813c-0.625 1.481-0.944 3.053-0.944 4.672h2c0-5.512 4.488-10 10-10 2.703 0 5.275 1.1 7.141 3z"></path>
    <path d="M4.859 19h4.141v-2h-6c-0.553 0-1 0.447-1 1v6h2v-3.059c0.725 0.647 1.525 1.209 2.384 1.666 1.719 0.912 3.659 1.394 5.616 1.394v-2c-2.703 0-5.275-1.1-7.141-3z"></path>
    <path d="M3 12h1c0-4.413 3.588-8 8-8v-1c-2.403 0-4.662 0.938-6.363 2.637s-2.637 3.959-2.637 6.363z"></path>
    <path d="M18 18h1v1h-1v-1z"></path>
    <path d="M18 13h1v4h-1v-4z"></path>
    <path d="M23.841 18.828l-4-7.997c-0.25-0.506-0.778-0.831-1.341-0.831s-1.091 0.325-1.341 0.828l-4 7.997c-0.103 0.206-0.159 0.438-0.159 0.672 0 0.828 0.672 1.5 1.5 1.5h8c0.828 0 1.5-0.672 1.5-1.5 0-0.231-0.056-0.462-0.159-0.669zM22.5 20h-8c-0.275 0-0.5-0.225-0.5-0.5 0-0.078 0.019-0.153 0.050-0.219l4.003-8.006c0.081-0.166 0.256-0.275 0.447-0.275s0.363 0.109 0.447 0.278l4.003 8.003c0.034 0.066 0.050 0.141 0.050 0.219 0 0.275-0.225 0.5-0.5 0.5z"></path>
  </svg>
);

SyncWarningIcon.propTypes = {
  className: PropTypes.string,
  size: PropTypes.number,
};

SyncWarningIcon.defaultProps = {
  size: 1.5,
};

export default SyncWarningIcon;
