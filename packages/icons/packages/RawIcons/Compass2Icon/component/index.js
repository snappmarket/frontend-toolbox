/**
 * THIS IS AN AUTO GENERATED FILE, CHANGES WILL NOT APPLY
 */
import * as React from 'react';
import PropTypes from 'prop-types';

const Compass2Icon = ({ className, size }) => (
  <svg
    data-testid="Compass2Icon"
    viewBox="0 0 24 24"
    className={className}
    style={{
      width: size * 10,
      height: size * 10,
    }}
    focusable="false"
    fill="currentColor"
  >
    <path d="M23.056 7.328c-0.603-1.428-1.469-2.712-2.572-3.813-1.103-1.103-2.384-1.966-3.813-2.572-1.481-0.625-3.053-0.944-4.672-0.944s-3.191 0.319-4.672 0.944c-1.428 0.603-2.712 1.469-3.813 2.572-1.103 1.103-1.966 2.384-2.572 3.813-0.625 1.481-0.944 3.053-0.944 4.672s0.319 3.191 0.944 4.672c0.603 1.428 1.469 2.712 2.572 3.813 1.103 1.103 2.384 1.966 3.813 2.572 1.481 0.625 3.053 0.944 4.672 0.944s3.191-0.319 4.672-0.944c1.428-0.603 2.712-1.469 3.813-2.572 1.103-1.103 1.966-2.384 2.572-3.813 0.625-1.481 0.944-3.053 0.944-4.672s-0.319-3.191-0.944-4.672zM12 22c-5.512 0-10-4.488-10-10s4.488-10 10-10c5.513 0 10 4.488 10 10s-4.488 10-10 10z"></path>
    <path d="M12 3c-2.403 0-4.666 0.938-6.363 2.637s-2.637 3.959-2.637 6.363 0.938 4.666 2.637 6.363c1.697 1.7 3.959 2.637 6.363 2.637s4.666-0.938 6.363-2.637c1.7-1.697 2.637-3.959 2.637-6.363s-0.938-4.666-2.637-6.363c-1.697-1.7-3.959-2.637-6.363-2.637zM12 20c-4.413 0-8-3.588-8-8s3.588-8 8-8c4.413 0 8 3.588 8 8s-3.588 8-8 8z"></path>
    <path d="M16.275 7.053l-6 3c-0.097 0.047-0.175 0.128-0.225 0.225l-3 6c-0.097 0.194-0.059 0.425 0.094 0.578 0.097 0.097 0.225 0.147 0.353 0.147 0.075 0 0.153-0.019 0.225-0.053l6-3c0.097-0.047 0.175-0.128 0.225-0.225l3-6c0.097-0.194 0.059-0.425-0.094-0.578s-0.384-0.191-0.578-0.094zM10.638 11.344l2.019 2.019-4.038 2.019 2.019-4.038zM13.362 12.656l-2.019-2.019 4.038-2.019-2.019 4.038z"></path>
  </svg>
);

Compass2Icon.propTypes = {
  className: PropTypes.string,
  size: PropTypes.number,
};

Compass2Icon.defaultProps = {
  size: 1.5,
};

export default Compass2Icon;
