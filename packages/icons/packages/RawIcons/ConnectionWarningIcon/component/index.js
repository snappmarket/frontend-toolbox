/**
 * THIS IS AN AUTO GENERATED FILE, CHANGES WILL NOT APPLY
 */
import * as React from 'react';
import PropTypes from 'prop-types';

const ConnectionWarningIcon = ({ className, size }) => (
  <svg
    data-testid="ConnectionWarningIcon"
    viewBox="0 0 24 24"
    className={className}
    style={{
      width: size * 10,
      height: size * 10,
    }}
    focusable="false"
    fill="currentColor"
  >
    <path d="M22 12h-4v-1h4.5c0.828 0 1.5-0.672 1.5-1.5 0-0.234-0.056-0.466-0.159-0.672l-4-7.997c-0.25-0.506-0.775-0.831-1.341-0.831s-1.091 0.325-1.341 0.828l-1.584 3.172h-2.575v-2c0-1.103-0.897-2-2-2h-9c-1.103 0-2 0.897-2 2v5c0 1.103 0.897 2 2 2h1.319l-0.306 1.834c-0.047 0.291 0.034 0.588 0.222 0.809s0.469 0.353 0.762 0.353h5c0.294 0 0.572-0.128 0.762-0.353s0.272-0.522 0.222-0.809l-0.306-1.834h1.325c1.103 0 2-0.897 2-2v-2h2.075l-1.913 3.828c-0.103 0.206-0.159 0.438-0.159 0.672 0 0.828 0.672 1.5 1.5 1.5h2.5v1h-4.003c-1.103 0-2 0.897-2 2v5c0 1.103 0.897 2 2 2h1.319l-0.306 1.834c-0.047 0.291 0.034 0.588 0.222 0.809 0.191 0.225 0.469 0.353 0.762 0.353h5c0.294 0 0.572-0.128 0.762-0.353s0.272-0.522 0.222-0.809l-0.306-1.834h1.325c1.103 0 2-0.897 2-2v-5c0-1.103-0.897-2-2-2zM7.988 11h-2.972l0.334-2h2.306l0.331 2zM11 7h-9v-5h9v5c0 0 0 0 0 0zM14 9.5c0-0.078 0.019-0.153 0.050-0.219l4.003-8.006c0.081-0.166 0.256-0.275 0.447-0.275s0.363 0.109 0.447 0.278l4.003 8.003c0.034 0.066 0.050 0.141 0.050 0.219 0 0.275-0.225 0.5-0.5 0.5h-8c-0.275 0-0.5-0.225-0.5-0.5zM18.987 23h-2.972l0.334-2h2.306l0.331 2zM22 19h-9v-5h9v5c0 0 0 0 0 0z"></path>
    <path d="M18 8h1v1h-1v-1z"></path>
    <path d="M18 3h1v4h-1v-4z"></path>
  </svg>
);

ConnectionWarningIcon.propTypes = {
  className: PropTypes.string,
  size: PropTypes.number,
};

ConnectionWarningIcon.defaultProps = {
  size: 1.5,
};

export default ConnectionWarningIcon;
