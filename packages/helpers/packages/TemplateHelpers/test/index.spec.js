...
describe("TemplateHelpers", () =>{
  describe("hexToRgb", () =>{
    it("should return zero as values of rgb because no hex was given", () =>{
      expect(TemplateHelpers.hexToRgb('')).toEqual({red: 0, green: 0, blue: 0});
    });
    it("should return rgb of given hex color", () =>{
      expect(TemplateHelpers.hexToRgb('#ffffff')).toEqual({red: 255, green: 255, blue: 255});
    });
  });
  ...
  describe('makeRgba', () => {
    it('should convert hex color to rgba color suitable for using in css', () => {
      expect(TemplateHelpers.makeRgba(0.5, '#ff00ff')).toEqual('rgba(255, 0, 255, 0.5)');
    });
  });
  ...
  describe('makeShadow', () => {
    it('should return inset shadow suitable for css', () => {
      expect(TemplateHelpers.makeShadow('10px', 1, '1em', 2, '#000000')).toEqual('10px calc(1rem * 1) 1em calc(1rem * 2) #000000');
    });
    it('should return inset shadow suitable for css', () => {
      expect(TemplateHelpers.makeShadow('10px', 1, '1em', 2, '#ffffff', true)).toEqual('10px calc(1rem * 1) 1em calc(1rem * 2) #ffffff inset');
    });
  });
});
